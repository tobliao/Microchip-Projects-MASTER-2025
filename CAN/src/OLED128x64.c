/* NOTE: When included in main.c, these are already defined there */
#ifndef OLED_STANDALONE
/* Used when compiled as separate object file */
#include <stddef.h>
#include <stdbool.h>
#include <stdlib.h>
...
#else
/* When included directly in main.c, just include the fonts */
#include "OLED_FONTS.c"
#endif

/* I2C wait with timeout - prevents hang if OLED not connected */
static inline void i2c_wait_with_timeout(void) {
    volatile uint32_t timeout = 100000;
    while(SERCOM2_I2C_IsBusy() && timeout > 0) {
        timeout--;
    }
}
...
void  OLEDWrCmd(uint8_t command)
{
    ...
    OLED_Data[1] = command;
    SERCOM2_I2C_Write(OLED_ADDRESS, OLED_Data, 2);
    i2c_wait_with_timeout();
}
